#!/usr/bin/env node
var PORT, Pomodoremi, _, args, bridge, cli, client, command, commands, commandsHelp, dnode, flags, header, maxCommandLength, meow, pkg, pomodoremi, s, subCommands,
  slice = [].slice;
...
if (command == null) {
  ...
  commandsHelp = _(commands).values().map(function(fn) {
    var description, ref;
    ref = fn(), command = ref[0], description = ref[1];
