var CONFIG_PATH, CommandLineLog, Notifier, Pomodoremi, Timer, Utils, _, config, s;
...
Pomodoremi = (function() {
  ...
  function Pomodoremi(options) {
    ...
    this.middlewares = [];
    this.middlewares.push(new CommandLineLog);
    this.middlewares.push(new Notifier);
    this.timer = new Timer();
  }

  Pomodoremi.prototype.start = function(name1) {
    this.name = name1 != null ? name1 : 'Pomodoro';
    this.resetTimer();
  ...
  Pomodoremi.prototype.startTimer = function(type, name) {
    var typeUppercase;
    this.type = type;
    this.startTime = new Date;
    typeUppercase = s(type).capitalize().value();
    this.middlewares[0].start(type, this.name, this[type + "Length"], function() {});
    this.timer.start(Utils.toMs(this[type + "Length"]));
    this.timer.update((function(_this) {
      return function(passed) {
        return _this.middlewares[0].update(type, passed, function() {});
      };
    })(this));
